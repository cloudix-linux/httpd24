...
%define docroot %{_localstatedir}/www
...
Source51: httpd.sysv.logrotate
Source52: httpd.sysconf.sysv
...
# Amazon patches
Patch80: httpd-2.2.21-expectnoka.patch
Patch81: amzn-layout.patch
Patch82: httpd-init.patch
Patch83: mod_rewrite-CVE-2013-1862.patch
License: ASL 2.0
...
Requires: /etc/mime.types, generic-logos >= 16.0.0
%if %{with systemd}
...
%description -n mod24_proxy_html
The mod_proxy_html and mod_xml2enc modules provide filters which can
transform and modify HTML and XML content.

%package -n mod24_ldap
Group: System Environment/Daemons
...
%description -n mod24_ldap
The mod_ldap and mod_authnz_ldap modules add support for LDAP
authentication to the Apache HTTP Server.

%package -n mod24_session
Group: System Environment/Daemons
...
%patch80 -p1 -b .expectnoka
%patch81 -p1 -b .amzn-layout
%patch82 -p1 -b .httpd-init
%patch83 -p1 -b .mod_rewrite-CVE-2013-1862
...
install -m755 build/rpm/httpd.init \
    $RPM_BUILD_ROOT%{_initrddir}/httpd
install -m755 build/rpm/htcacheclean.init \
    $RPM_BUILD_ROOT%{_initrddir}/htcacheclean
%endif
...
for f in welcome.conf ssl.conf manual.conf userdir.conf notrace.conf; do
  install -m 644 -p $RPM_SOURCE_DIR/$f \
...
%else
install -m 644 -p $RPM_SOURCE_DIR/httpd.sysv.logrotate \
	$RPM_BUILD_ROOT/etc/logrotate.d/httpd
%endif
...
sed -e "s|/usr/local/apache2/conf/httpd.conf|/etc/httpd/conf/httpd.conf|" \
    ...
    -e "s|/usr/local/apache2/logs/error_log|%{_localstatedir}/log/httpd/error_log|" \
    -e "s|/usr/local/apache2/logs/access_log|%{_localstatedir}/log/httpd/access_log|" \
...
%define sslconf %{_sysconfdir}/pki/tls/openssl.cnf
%define temp_sslconf %{_tmppath}/openssl-localhost.cnf
...
EOF
    ...
    cat << EOF | %{_bindir}/openssl req -new -key %{sslkey} \
         ...
         -out %{sslcert} -config %{temp_sslconf} 2>/dev/null
--
SomeState
SomeCity
SomeOrganization
...
subjectAltName="DNS:localhost,DNS:localhost.localdomain,DNS:$(hostname),DNS:$(hostname -f)"
genCert ${subjectAltName}

if [ ! -r %{sslcert} ]; then
    # hostname -f fails on systems that can't resolve their own DNS name
    # Try again without it.
    subjectAltName="DNS:localhost,DNS:localhost.localdomain,DNS:$(hostname)"
    genCert ${subjectAltName}
...
if [ ! -r %{sslcert} ]; then
    # If the second attempt failed, abort the install,
    # otherwise httpd will be rendered unable to start.
    echo "SSL certificate generation failed, cannot proceed."
    exit 1
fi
...
%{_sbindir}/apachectl
%{_sbindir}/rotatelogs
# This way causes issues with older versions of RPM
...
%changelog
* Wed Mar 18 2015 hpetty <hpetty@amazon.com>
- Updated to upstream apache 2.4.12

* Thu Mar 12 2015 Rodrigo Novo <rodarvus@amazon.de>
- Fix for expat 2.1.0

* Thu Feb 12 2015 hpetty <hpetty@amazon.com>
- openssl does not support PROFILE=SYSTEM option for SSLCipherSuite

* Wed Feb 11 2015 hpetty <hpetty@amazon.com>
- import source package F21/httpd-2.4.10-15.fc21
- import source package F21/httpd-2.4.10-9.fc21
- import source package F21/httpd-2.4.10-7.fc21
- import source package F21/httpd-2.4.10-6.fc21
...
* Mon Jul 28 2014 hpetty <hpetty@amazon.com>
- import source package F21/httpd-2.4.10-1.fc21
- import source package F21/httpd-2.4.9-8.fc21
- import source package F21/httpd-2.4.9-7.fc21
...
* Tue Apr 1 2014 hpetty <hpetty@amazon.com>
- updated requires for apr
...
* Wed Mar 19 2014 cyler <cyler@amazon.com>
- import source package F21/httpd-2.4.9-1.fc21
- import source package F19/httpd-2.4.7-1.fc19

* Tue Mar 18 2014 cyler <cyler@amazon.com>
- Hide stdout from service stop commands in %%preun
- chkconfig --add htcacheclean on install
- Move condrestart to %%postun
- chkconfig --add on install
...
* Thu Sep 19 2013 Jamie Anderson <jamieand@amazon.com>
- Add fallback SSL cert generation for hosts without valid DNS entries

* Wed Sep 11 2013 cyler <cyler@amazon.com>
- fix issue with logrotate calling systemctl on SysV systems

* Mon Aug 12 2013 Lee Trager <ltrager@amazon.com>
- import source package F18/httpd-2.4.6-2.fc18
...
* Thu Apr 4 2013 cyler <cyler@amazon.com>
- modify spec to generate localhost cert with CA:TRUE and alternate subject names

* Tue Mar 26 2013 cyler <cyler@amazon.com>
- make httpd24 provide versioned mod_session

* Tue Mar 26 2013 Jan Kaluza <jkaluza@redhat.com> - 2.4.4-3
- protect MIMEMagicFile with IfModule (#893949)

* Fri Mar 22 2013 cyler <cyler@amazon.com>
- reference _localstatedir instead of /var
- reference _localstatedir/run not run/

* Thu Mar 21 2013 cyler <cyler@amazon.com>
- import source package F18/httpd-2.4.4-2.fc18
- import source package F18/httpd-2.4.3-15.fc18
- import source package F18/httpd-2.4.3-12.fc18
...
* Mon Nov 12 2012 Lee Trager <ltrager@amazon.com>
- Only call systemd on systems with systemd installed

* Fri Oct 26 2012 Joe Orton <jorton@redhat.com> - 2.4.3-12
- rebuild to pick up new apr-util-ldap

* Tue Oct 23 2012 Lee Trager <ltrager@amazon.com>
- Patch init script to look for pid files in /var/run/httpd
- Only enable mod_systemd if the system is running systemd

* Tue Oct 23 2012 Joe Orton <jorton@redhat.com> - 2.4.3-11
- rebuild

* Mon Oct 22 2012 Lee Trager <ltrager@amazon.com>
- httpd pid path corrected upstream, patch no longer needed
- import source package F18/httpd-2.4.3-10.fc18
...
* Thu Aug 23 2012 Lee Trager <ltrager@amazon.com>
- Rename to httpd24
...
* Wed Aug 15 2012 Lee Trager <ltrager@amazon.com>
- Revert to the old way setting suid on suexec

* Fri Aug 10 2012 Lee Trager <ltrager@amazon.com>
- import source package F18/httpd-2.4.2-23.fc18
- Conidtionalize systemd support add in patches to support the Amazon Linux AMI

* Mon Aug  6 2012 Joe Orton <jorton@redhat.com> - 2.4.2-23
- add mod_proxy fixes from upstream (r1366693, r1365604)

* Wed Jul 25 2012 Lee Trager <ltrager@amazon.com>
- import source package F18/httpd-2.4.2-22.fc18
